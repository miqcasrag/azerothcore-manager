...
#    unique!"
#
# This function cleans up any such duplicates for EVERY
# installed module (Playerbots and any module from $MODULES),
# not just Playerbots, since the same bug can hit any module
# that follows this layout. It only removes a custom/ file
# when a module directory provides a file with the exact same
# name, so genuine custom SQL you placed yourself is left
...
prepare_playerbots_config() {
    ...
    if [[ ! -f "$config_dir/playerbots.conf" ]]; then
        ...
        success "Playerbots configuration created."
...
PLAYERBOTS_OVERRIDE_MARKER="# managed-by: ac-manager.sh (playerbots modules mount)"
...
write_playerbots_docker_override() {
    ...
    if [[ -f "$override_file" ]]; then
        ...
        warning "docker-compose.override.yml already exists but doesn't mount modules/."
        echo
        echo "  $override_file"
        echo
...
EOF
    ...
    success "docker-compose.override.yml created (mounts modules/ into ac-worldserver)."
