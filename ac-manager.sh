...
run_build() {
    ...
    while kill -0 "$pid" 2>/dev/null; do
        ...
        echo
        echo -e "${BOLD}${operation}${RESET}   ${CYAN}$(printf '%02d:%02d' "$minutes" "$seconds")${RESET}"
        echo "────────────────────────────────────────"
        echo

        tail -n "$BUILD_LINES" "$tmp_file"
    ...
    echo
    echo -e "${BOLD}${operation}${RESET}   ${CYAN}$(printf '%02d:%02d' "$total_minutes" "$total_seconds")${RESET}"
    echo "────────────────────────────────────────"
    echo
