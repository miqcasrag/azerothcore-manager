...
install_override() {
    ...
    cp "$OVERRIDE_FILE" "$CORE_DIR/docker-compose.override.yml"
...
fix_permissions() {
    ...
    expected_core="$(realpath "$SCRIPT_DIR/azerothcore-wotlk")"
    real_core="$(realpath "$CORE_DIR")"

    if [[ "$real_core" != "$expected_core" ]]; then
        error "CORE_DIR safety check failed."
        return 1
    fi

    echo
    info "Updating ownership:"
    echo "  $real_core"
    echo

    sudo chown -R 1000:1000 "$real_core"
...
run_build() {
    ...
    (
        ...
        docker compose build --progress=plain >"$log_file" 2>&1
    ) &
    ...
    while kill -0 "$pid" 2>/dev/null; do
        ...
        echo "────────────────────────────────"
        echo
        echo -e "Building... ${CYAN}$(printf '%02d:%02d' "$minutes" "$seconds")${RESET}"
        echo
...
rebuild_server() {
    ...
    echo "────────────────────────────────"
    echo
    echo "This only recompiles the Docker images."
...
module_status() {
    ...
    for module in "${MODULES[@]}"; do
        ...
        if [[ -d "$CORE_DIR/modules/$name" ]]; then
            ...
            printf "  %-2s %-28s ${GREEN}● installed${RESET}\n" \
                "$number" "$name"

        else

            printf "  %-2s %-28s ${DIM}○ not installed${RESET}\n" \
                "$number" "$name"
...
update_core() {
    ...
    latest_commit="$(git -C "$CORE_DIR" rev-parse "origin/$CORE_BRANCH")"
    ...
    echo
    echo "The following local data will be preserved:"
    echo "  ✓ .env"
    ...
    # Preserve local configuration files that are managed locally.
    local env_backup
    ...
    if [[ -f "$CORE_DIR/docker-compose.override.yml" ]]; then
        cp "$CORE_DIR/docker-compose.override.yml" "$override_backup"
    fi

    # Update only tracked core files.
    git -C "$CORE_DIR" checkout -- .

    if ! git -C "$CORE_DIR" merge --ff-only "origin/$CORE_BRANCH"; then

        error "Failed to update AzerothCore."

        cp "$env_backup" "$CORE_DIR/.env" 2>/dev/null || true
        cp "$override_backup" \
            "$CORE_DIR/docker-compose.override.yml" 2>/dev/null || true
    ...
    case "$choice" in
        ...
        1)
            ...
            if [[ -d "$CORE_DIR/volumes" ]]; then
                mv "$CORE_DIR/volumes" "$temp_dir/volumes"
            fi
