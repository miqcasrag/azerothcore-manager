...
- **Remote backup (rclone)** — Optionally install rclone and upload each
  backup to a cloud/remote storage target, with its own separate
  retention limit so the remote doesn't fill up either.
