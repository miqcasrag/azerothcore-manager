...
has_connections() {
    ...
    # A player is "connected" during any of these phases:
    #   1. Login / char-select  → established connection on PROXY_LISTEN_PORT (3724)
    #   2. In-world             → established connection on WORLD_PORT (8085)
    # We must watch BOTH so we don't count idle time while a player
    # is still on the login screen or selecting a character.

    local count_auth count_world

    # Auth / login port (our proxy in front of ac-authserver) --
    # this is our own host process, always reliable.
    count_auth="$(ss -Htn state established "sport = :${PROXY_LISTEN_PORT}" 2>/dev/null | wc -l)"
    [[ "${count_auth:-0}" -gt 0 ]] && return 0

    # World port -- fast path via host ss. Whether this actually
    # sees anything depends on Docker's port-publishing mode
    # (userland-proxy vs iptables-only), so it's a best-effort
    # shortcut, not something to rely on alone.
    count_world="$(ss -Htn state established "sport = :${WORLD_PORT}" 2>/dev/null | wc -l)"
    [[ "${count_world:-0}" -gt 0 ]] && return 0

    # Reliable fallback: ask the container itself, from inside its
    # own network namespace, so it doesn't matter how Docker is
    # publishing the port on the host.
    local count_inner

    # Prefer ss inside the container if it has iproute2 -- simplest
    # and handles IPv4/IPv6 transparently.
    count_inner="$(
        compose exec -T ac-worldserver \
            sh -c "ss -Htn state established '( sport = :${WORLD_PORT} )' 2>/dev/null | wc -l" \
            2>/dev/null
    )"

    if [[ -n "$count_inner" ]]; then
        [[ "$count_inner" -gt 0 ]] && return 0
        return 1
    fi

    # ss isn't available in the image -- parse /proc/net/tcp AND
    # /proc/net/tcp6 by hand (a dual-stack bind shows up in the v6
    # table even for IPv4 clients).
    local port_hex
    port_hex="$(printf '%04X' "$WORLD_PORT")"
